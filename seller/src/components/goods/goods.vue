...
//              console.log(index); // 只执行派发的点击事件
          ...
          selectFood: function (food, event) {
            ...
            this.selectedFood = food;  // 全局维护selectedFood => 点击当前food,把该对象传给selectedFood => 通过props下放到food详情组件来填充/计算内容

            // 点击显示food详情层
            this.$refs.food.show(); // 父组件调用子组件方法(首先用vue的一个接口拿到子组件,进而调用它的方法)
          }
      },
      components: {
          shopcart,
